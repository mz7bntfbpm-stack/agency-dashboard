...
const ImportData = () => {
  ...
  const validateFile = (file) => {
    ...
    if (!validTypes.includes(file.type) && extension !== 'csv') {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setImportStatus({
        type: 'success',
        message: `Successfully imported ${files.length} file(s). Data has been processed and updated.`
      })
      setFiles([])
    } catch (error) {
      setImportStatus({
        type: 'error',
        message: error.message || 'Failed to import files. Please check the file format and try again.'
      })
    } finally {
      setImporting(false)
    }
  ...
  const downloadTemplate = (platform) => {
    const templates = {
      ga4: ['Date', 'Sessions', 'Users', 'Conversions', 'Revenue', 'Source', 'Medium'],
      google_ads: ['Date', 'Campaign', 'Impressions', 'Clicks', 'Spend', 'Conversions', 'CostPerConversion'],
      facebook: ['Date', 'Campaign Name', 'Reach', 'Impressions', 'Clicks', 'Spend', 'Results'],
      instagram: ['Date', 'Post', 'Impressions', 'Reach', 'Engagement', 'Saves', 'Shares'],
    }
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              {platforms.map((platform) => (
                ...
                >
                  ...
                  <span className={`text-sm font-medium ${
                    selectedPlatform === platform.id ? 'text-blue-400' : 'text-white'
                  }`}>
                    {platform.name}
                  </span>
                </button>
          ...
          {/* File Drop Zone */}
          <div
            className={`bg-dark-800/50 border-2 border-dashed rounded-xl p-8 transition-all ${
              dragActive 
          ...
          >
            <div className="text-center">
              <div className={`w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center ${
                dragActive ? 'bg-blue-500' : 'bg-dark-700'
              }`}>
              ...
              <label
                htmlFor="file-upload"
                className="inline-flex items-center gap-2 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-medium cursor-pointer transition-colors"
              >
                  ...
                  return (
                    <div 
                      ...
                      className={`flex items-center justify-between p-3 rounded-lg ${
                        validation.valid ? 'bg-dark-700/30' : 'bg-red-500/10 border border-red-500/30'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            ...
                            {!validation.valid && (
                              <span className="text-red-400 ml-2">- {validation.error}</span>
                            )}
                          </p>
                        </div>
                      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-dark-800/50 border border-dark-700 rounded-xl p-5">
            ...
            <div className="space-y-2">
              {platforms.map((platform) => (
                <button
                  key={platform.id}
                  onClick={() => downloadTemplate(platform.id)}
                  className="w-full flex items-center justify-between p-3 rounded-lg bg-dark-700/30 hover:bg-dark-700 text-left transition-colors"
                >
                  <div className="flex items-center gap-3">
          ...
          <div className="bg-dark-800/50 border border-dark-700 rounded-xl p-5">
            ...
            <div className="space-y-4">
              <div className="p-3 rounded-lg bg-dark-700/30">
                ...
                <code className="text-xs text-blue-400 bg-dark-800 px-2 py-1 rounded block">
                  https://api.agencymetrics.com/webhooks/ads
                </code>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <Cloud className="w-4 h-4 text-emerald-400" />
                <span className="text-slate-400">Receive real-time data updates</span>
              </div>
              <button className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-dark-700 hover:bg-dark-600 text-white rounded-lg text-sm font-medium transition-colors">
          ...
          <div className="bg-dark-800/50 border border-dark-700 rounded-xl p-5">
            ...
            <ul className="space-y-2 text-sm text-slate-400">
              ...
              <li className="flex items-start gap-2">
                <CheckCircle className="w-4 h-4 text-emerald-400 flex-shrink-0 mt-0.5" />
                <span>Supported platforms: GA4, Google Ads, Facebook, Instagram</span>
              </li>
            </ul>
          </div>
        </div>
